...
function extractAddressFields(text) {
  ...
  if (enderecoMatch) {
    result.endereco = cleanText(enderecoMatch[1]);
  }
...
function extractEnergyData(text) {
  ...
  const address = extractAddressFields(source);
...
function applyExtractedData({ billData, cnhData }) {
  ...
  enderecoOutorganteInput.value = billData.endereco || enderecoOutorganteInput.value;
...
async function init() {
  ...
  currentUserEmail = await resolveCurrentUser();
