...
function renderInvoice(data) {
  ...
  const valorEnergiaNaoCompensada = energiaNc * tarifaRef;
    ...
    <article class="invoice-j7">
      ...
      <section class="j7-client">
        <div class="j7-grid">
          ...
          <div><b>Endereco:</b> ${escapeHtml(address)}</div>
        ...
        <div class="j7-columns">
          ...
          <div class="j7-right">
            ...
            <div class="j7-panel white">
              ...
              <div class="j7-table-head"><span></span><span>QUANTIDADE</span><span>VALOR A PAGAR</span></div>
              <div class="j7-trow"><span>Energia eletrica nao compensada</span><span>${kwhCompact(energiaNc)}</span><span>${brl(valorEnergiaNaoCompensada)}</span></div>
...
async function renderOriginalInvoice(file) {
  ...
  if (type.includes("pdf") || file.name.toLowerCase().endsWith(".pdf")) {
    ...
    try {
      ...
      originalInvoiceHost.innerHTML = '<p class="preview-empty">Nao foi possivel renderizar a fatura original.</p>';
...
uploadInput?.addEventListener("change", () => {
  ...
  postInvoiceToV2(file)
    ...
    .catch((error) => {
      ...
      invoiceRenderHost.innerHTML = `<p class="preview-empty">Nao foi possivel gerar a fatura: ${escapeHtml(error.message)}</p>`;
      lastExtractData = null;
      setValidationButtonState(false, false);
    });
});
